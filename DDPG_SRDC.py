...
class DDPG_SRDC(object):
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            target_Q, _ = self.critic_target(next_state, next_action, 0)
            target_Q = reward + not_done * self.discount * target_Q

        # Get current Q estimates
        current_Q1, current_mean1 = self.critic(state, action, self.num_sample)
        baseline = torch.zeros_like(current_mean1)
        # Compute critic loss
        Q_loss = F.mse_loss(current_Q1, target_Q)
        regular_loss = F.mse_loss(current_mean1, baseline)
        if self.adaptive:
            with torch.no_grad():
        ...
        return float(Q_loss), float(regular_loss), float(v), float(-actor_loss), True
